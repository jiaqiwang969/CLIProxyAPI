...
func TestMain(m *testing.M) {
	loadAuggieSessionFile = func() (*sdkAuth.AuggieSession, error) {
...
func TestSnapshotCoreAuths_IncludesAuggieSessionWhenNoAuthFileExists(t *testing.T) {
	...
	auth := auths[0]
	if auth.Provider != "auggie" {
		t.Fatalf("provider = %q, want auggie", auth.Provider)
	}
	if auth.ID != "auggie-tenant-augmentcode-com.json" {
		t.Fatalf("id = %q, want auggie-tenant-augmentcode-com.json", auth.ID)
	}
	if auth.Label != "tenant.augmentcode.com" {
		t.Fatalf("label = %q, want tenant.augmentcode.com", auth.Label)
	}
	if got := auth.Attributes["runtime_only"]; got != "true" {
		t.Fatalf("runtime_only = %q, want true", got)
	}
	if got := auth.Attributes["auth_kind"]; got != "oauth" {
		t.Fatalf("auth_kind = %q, want oauth", got)
	}
	expectedHash := diff.ComputeExcludedModelsHash([]string{"gpt-5-4"})
	if got := auth.Attributes["excluded_models_hash"]; got != expectedHash {
		t.Fatalf("excluded_models_hash = %q, want %q", got, expectedHash)
	}
...
func TestSnapshotCoreAuths_MergesAuggieSessionIntoSameTenantAuthFile(t *testing.T) {
	useRealAuggieSessionLoader(t)
	...
	authDir := t.TempDir()
	authFile := filepath.Join(authDir, "auggie-tenant-augmentcode-com.json")
	metadata := map[string]any{
		"type":         "auggie",
		"label":        "Team Alpha",
	...
	auths := w.SnapshotCoreAuths()
	if len(auths) != 1 {
		t.Fatalf("expected merged auth result, got %d auths", len(auths))
	}
	...
	if got := auth.Attributes["source"]; got != authFile {
		t.Fatalf("source = %q, want %q", got, authFile)
	}
	if got := auth.Metadata["access_token"]; got != "session-token" {
		t.Fatalf("access_token = %v, want session-token", got)
	}
...
func TestSnapshotCoreAuths_UpdatesAllSameTenantAuggieAuthFilesFromSession(t *testing.T) {
	useRealAuggieSessionLoader(t)
	...
	authDir := t.TempDir()
	writeAuggieWatcherAuthFile(t, filepath.Join(authDir, "auggie-tenant-a.json"), map[string]any{
		"type":         "auggie",
		"tenant_url":   "https://tenant.augmentcode.com/",
		"access_token": "stale-token-a",
	})
	writeAuggieWatcherAuthFile(t, filepath.Join(authDir, "auggie-tenant-b.json"), map[string]any{
		"type":         "auggie",
		"tenant_url":   "https://tenant.augmentcode.com/",
		"access_token": "stale-token-b",
	})
	...
	for _, auth := range auths {
		if got := auth.Metadata["access_token"]; got != "session-token" {
			t.Fatalf("auth %q access_token = %v, want session-token", auth.ID, got)
		}
	}
}
...
func writeWatcherAuggieSessionFile(t *testing.T, homeDir, body string) {
	t.Helper()
...
func writeAuggieWatcherAuthFile(t *testing.T, path string, metadata map[string]any) {
	t.Helper()
...
func useRealAuggieSessionLoader(t *testing.T) {
	t.Helper()

	previous := loadAuggieSessionFile
	loadAuggieSessionFile = sdkAuth.LoadAuggieSessionFile
	t.Cleanup(func() {
		loadAuggieSessionFile = previous
	})
}
