...
func parseAuggieUsage(data []byte) (usage.Detail, bool) {
	...
	if input := root.Get("prompt_tokens"); input.Exists() {
		detail.InputTokens = input.Int()
		found = true
	} else if input := root.Get("input_tokens"); input.Exists() {
		detail.InputTokens = input.Int()
		found = true
	}

	if output := root.Get("completion_tokens"); output.Exists() {
		detail.OutputTokens = output.Int()
		found = true
	} else if output := root.Get("output_tokens"); output.Exists() {
		detail.OutputTokens = output.Int()
		found = true
	}

	if total := root.Get("total_tokens"); total.Exists() {
		detail.TotalTokens = total.Int()
		found = true
	}

	if cached := root.Get("cached_tokens"); cached.Exists() {
		detail.CachedTokens = cached.Int()
		found = true
	}

	if reasoning := root.Get("reasoning_tokens"); reasoning.Exists() {
		detail.ReasoningTokens = reasoning.Int()
		found = true
	} else if reasoning := root.Get("thinking_tokens"); reasoning.Exists() {
		detail.ReasoningTokens = reasoning.Int()
		found = true
	}
